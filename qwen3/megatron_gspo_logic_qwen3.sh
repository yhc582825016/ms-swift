...
export SWANLAB_EXPERIMENT_NAME="Qwen3-4B-gspo-logic-rl"
...
megatron rlhf \
    --rlhf_type grpo \
    --model /opt/users/models/Qwen3-4B \
    --model_type qwen3 \
    --template qwen3_nothinking \
    --dataset "${TRAIN_DATASET}" \
    --val_dataset "${VAL_DATASET}" \
    --external_plugins "${MS_SWIFT_ROOT}/examples/train/grpo/plugin/logic_rl_reward_plugin.py" \
    ...
    --output_dir "${MS_SWIFT_ROOT}/megatron_output/Qwen3-4B-gspo-logic-rl" \
    > "/mnt/code/yehangcheng/ms-swift/megatron_output/logs/Qwen3-4B-gspo-logic-rl.log" 2>&1 &
